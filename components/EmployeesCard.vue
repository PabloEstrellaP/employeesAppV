          ...
          <v-card-text>
            ...
            >
              ...
              <template v-slot:[`item.action`]="{ item }">
                <v-icon
                  class="mr-2"
                  @click="$refs.employeeAddCard.getEmployee(item)"
                  >mdi-pencil</v-icon
                >
                <v-icon
                  class="mr-2"
                  @click="$refs.employeeDeleteDialog.openDialog(item)"
                  >mdi-delete</v-icon
                >
  ...
  methods: {
    ...
    printEmploye(employee) {
      try {
        ...
        doc.text(
          `Mérida, Yucatán a ${day} de ${this.getMonth(month)} del año ${year}`,
          80,
          10
        );
        doc.text(`A QUIEN CORRESPONDA:`, 10, 30);
        doc.text(
          `Por este medio de la presente y para los fines que pretenda el interesado,`,
          10,
          50
        );
        doc.text(
          `hago de su conocimiento ampliamente, al C. ${employee.name}, ya que es una`,
          10,
          60
        );
        doc.text(
          `persona Honesta y Responsable en las actividades que durante el periodo`,
          10,
          70
        );
        doc.text(
          `que prestó servicios en nuestra empresa le fueron asignadas, por tal motivo`,
          10,
          80
        );
        doc.text(
          `no tengo ninguna duda en expedir esta recomendación.`,
          10,
          90
        );
        doc.text(
          `Se extiendo la presente a solicitud del interesado y para los fines que`,
          10,
          110
        );
